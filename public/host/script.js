...
socket.on('config_updated', (data) => {
    if (data.difficulty) {
        difficultyLabel.textContent = data.difficulty === 'EASY' ? 'ŁATWY 🌸' : 'TRUDNY 🔥';
    }
...
socket.on('difficulty_changed', (level) => {
    difficultyLabel.textContent = level === 'EASY' ? 'ŁATWY 🌸' : 'TRUDNY 🔥';
});
...
function renderGrid(grid) {
    ...
    // 4x4: 18vh, gap 1.5vh (default in CSS)
    // 5x5: 14vh, gap 1.2vh
    // 6x6: 11vh, gap 1.0vh
...
async function runWordReview(results) {
    ...
    if (!results || results.length === 0) {
        reviewContainer.innerHTML = '<p style="color:#888;font-size:1.5rem;">Brak wyników</p>';
    ...
    sharedTitle.textContent = '⚔️ Powtarzające się słowa';
    ...
    sharedTitle.textContent = '⭐ Unikalne słowa (×2 punkty!)';
...
async function addWordToReview(player, wordData, type) {
    ...
    // wordEl.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); // No longer needed for single word?
...
function renderFinalTable(results) {
    ...
    if (!results || results.length === 0) {
        finalScores.innerHTML = '<p style="color:#888;font-size:1.5rem;">Brak wyników</p>';
        return;
    }
